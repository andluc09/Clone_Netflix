...
function MoviePopUp(props) {
  ...
  const fetchCast = async () => {
    const response = await fetch(`https://api.themoviedb.org/3/movie/${props.data1.id}/casts?api_key=${API_KEY}&language=pt-BR`);
                  ...
                  <Fade bottom>
                    <div className="p-5 py-4 -mb-6 mt-2 sm:mb-0 sm:mt-0 sm:py-0 sm:pt-6 rounded-t">
                      ...
                      <h1 className="text-green-700 font-bold mt-2">
                        {String(PopupInfo.release_date).split('-').reverse().join('-')}
                      </h1>
                    </div>
                  </Fade>
                  {/*body*/}
                      ...
                      <div className="relative p-2 py-5 sm:p-6 flex-auto">
                        ...
                        <h1 className="flex text-neutral-400 text-sm leading-relaxed">
                          ...
                          <p className="text-white ml-2 font-medium">
                            {String(PopupInfo.release_date || PopupInfo.first_air_date).split('-').reverse().join('-')}
                          </p>
                        </h1>
                        <h1 className="flex text-neutral-400 text-sm leading-relaxed">
                          Idioma :
                        ...
                        <h1 className="flex text-neutral-400 text-sm leading-relaxed">
                          ...
                          <p className="text-white ml-2 font-medium">
                          {PopupInfo.genre_ids &&
                          convertGenere(PopupInfo.genre_ids).reduce((acc, genere, index) => {
                            const concatenatedString = acc + genere + (index < convertGenere(PopupInfo.genre_ids).length - 1 ? ', ' : '');
                            if (concatenatedString.length > 50) return acc + '...';
                        ...
                        <h1 className="flex text-neutral-400 text-sm leading-relaxed">
                          Elenco :
                          <p className="text-white ml-2 font-medium">
                            {cast.slice(0, 3).map((actor) => actor.original_name).join(', ').substring(0, 45).trim() + (cast.length > 3 ? '...' : '')}
                          </p>
                        </h1>
                      </div>
                    </Fade>
